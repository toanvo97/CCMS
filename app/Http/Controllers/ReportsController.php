...
class ReportsController extends Controller
{
    ...
    */
    ...
    public function getHeader($request){
        ...
        if (($request->filled('total_cost')) || ($request->filled('depreciation'))) {
            $header[] = trans('admin/hardware/table.total_cost');
        }

        if (($request->filled('use_bom') || $request->filled('use_bom_model') || $request->filled('use_bom_location')) && $request->filled('total_cost')) {
            $header[] = trans('admin/hardware/table.sum_total');
        }
        ...
        if ($request->filled('warranty')) {
            $header[] = trans('admin/hardware/table.warranty');
            $header[] = trans('admin/hardware/table.warranty_expired');
        }
    ...
    public function getAssetCustom($request){
        if(isset($request->use_bom)){
            $assets = \App\Models\Company::scopeCompanyables(Asset::select('assets.*', DB::raw('SUM(order_number) as order_number_total')))
            ->groupBy('assets.company_id', 'assets.name', 'assets.purchase_cost', 'assets.purchase_date')->with(
                'location', 'assetstatus', 'assetlog', 'company', 'defaultLoc', 'assignedTo',
                'model.category', 'model.manufacturer', 'supplier')->orderBy('assets.name');
        }
        else if(isset($request->use_bom_model)){
            $assets = \App\Models\Company::scopeCompanyables(Asset::select('assets.*', DB::raw('SUM(order_number) as order_number_total')))
            ->groupBy('assets.company_id', 'assets.name', 'assets.purchase_cost', 'assets.purchase_date', 'assets.model_id')->with(
                'location', 'assetstatus', 'assetlog', 'company', 'defaultLoc', 'assignedTo',
                'model.category', 'model.manufacturer', 'supplier')->orderBy('assets.model_id')->orderBy('assets.company_id')->orderBy('assets.name');
        }
        else if(isset($request->use_bom_location)){
            $assets = \App\Models\Company::scopeCompanyables(Asset::select('assets.*', DB::raw('SUM(order_number) as order_number_total')))
            ->groupBy('assets.company_id', 'assets.name', 'assets.purchase_cost', 'assets.purchase_date', 'assets.location_id')->with(
                'location', 'assetstatus', 'assetlog', 'company', 'defaultLoc', 'assignedTo',
                'model.category', 'model.manufacturer', 'supplier')->orderBy('assets.location_id')->orderBy('assets.model_id')->orderBy('assets.company_id')->orderBy('assets.name');
        }
        else{
        ...
        if ($request->filled('by_asset_name')) {
            $assets->where('assets.name','LIKE', '%' . $request->input('by_asset_name') . '%');
        }
        ...
        if ($request->filled('by_notes')) {
            $assets->where('assets.notes', $request->input('by_notes'));
        }
        ...
        $data        = $assets->orderBy('assets.id')->get();
        $result      = [];
        $totalCostId = '';
        $orderNumber        = '';
        $totalOrderNumber   = 0;
        $totalCost          = 0;
            ...
            foreach ($data as $asset) {
                ...
                $row[] = $count++;
                if ($request->filled('company')) {
                    $row['company'] = ($asset->company) ? $asset->company->name : '';
                }

                if ($request->filled('asset_name')) {
                    $row['asset_name'] = ($asset->name) ? $asset->name : '';
                }
                ...
                if ($request->filled('model')) {
                    $row['model'] = ($asset->model) ? $asset->model->name : '';
                    // $row[] = ($asset->model) ? $asset->model->model_number : '';
                }

                if ($request->filled('category')) {
                    $row['category'] = (($asset->model) && ($asset->model->category)) ? $asset->model->category->name : '';
                }
                ...
                if ($request->filled('purchase_date')) {
                    ...
                    $row[] = ($asset->purchase_date) ? Helper::getFormattedDateObject($asset->purchase_date, '')['formatted'] : '';
                }
                
                if ($request->filled('order')) {
                    $row[] = isset($asset->order_number_total) ? $asset->order_number_total : (($asset->order_number) ? $asset->order_number : '0');
                    $orderNumber = count($row);
                    $orderNumberTotal = isset($asset->order_number_total) ? $asset->order_number_total : (($asset->order_number) ? $asset->order_number : '0');
                    $totalOrderNumber += $orderNumberTotal;
                    
                }

                if ($request->filled('purchase_cost')) {
                    $row[] = ($asset->purchase_cost) ? $asset->purchase_cost : '0';
                }

                if ($request->filled('total_cost')) {
                    $row['total_cost'] = isset($asset->order_number_total) && isset($asset->purchase_cost) ? $asset->order_number_total * $asset->purchase_cost : 
                        (($asset->order_number) && ($asset->purchase_cost) ? $asset->order_number * $asset->purchase_cost : '0');
                        $totalCostId = count($row);
                        $total = isset($asset->order_number_total) ? $asset->order_number_total * $asset->purchase_cost : 
                            (($asset->order_number) ? $asset->order_number * $asset->purchase_cost : '0');
                        $totalCost += $total;
                ...
                if ($request->filled('rtd_location')) {
                    $row['location'] = ($asset->defaultLoc) ? $asset->defaultLoc->present()->name() : '';
                }
                ...
                if ($request->filled('created_at')) {
                    $row[] = ($asset->created_at) ?  Helper::getFormattedDateObject($asset->created_at, '')['formatted'] : '';
                }

                if ($request->filled('updated_at')) {
                    $row[] = ($asset->updated_at) ?  Helper::getFormattedDateObject($asset->updated_at, '')['formatted'] : '';
                }

                if ($request->filled('last_audit_date')) {
                    $row[] = ($asset->last_audit_date) ? Helper::getFormattedDateObject($asset->last_audit_date, '')['formatted'] : '';
                }

                if ($request->filled('next_audit_date')) {
                    $row[] = ($asset->next_audit_date) ? Helper::getFormattedDateObject( $asset->next_audit_date , '')['formatted'] : '';
                }
                ...
                //         $row[] = ($asset->$column_name) ? $asset->$column_name : '';
                //     }
                // }
                $result[] = $row;
            }
            $mergeRows = [];
            $dataToSum = ['total_order_number' => $totalOrderNumber, 'total_cost' => $totalCost, 'order_number_column_id' => $orderNumber];
            foreach($result as $key => $data){
                if($request->use_bom_model){
                    $name = $data['model'];
                }
                else if($request->use_bom_location){
                    $name = $data['location'];
                }
                else{
                    $name = $data['asset_name'];
                }
                if(isset($mergeRows[$name])){
                    if(isset($mergeRows[$name][$data['company']][$data['category']])) {
                        $mergeRows[$name][$data['company']][$data['category']]['total_cost_column_id']    = $totalCostId;
                        array_push($mergeRows[$name][$data['company']][$data['category']]['array_ids'], $data[0]);
                        $dataTotalCost = !empty($data['total_cost']) ? $data['total_cost'] : 0;
                        $mergeRows[$name][$data['company']][$data['category']]['totals'] += $dataTotalCost;
                    }else{
                        $mergeRows[$name][$data['company']][$data['category']]['total_cost_column_id']    = $totalCostId;
                        $mergeRows[$name][$data['company']][$data['category']]['array_ids'][]             = $data[0];
                        $mergeRows[$name][$data['company']][$data['category']]['totals']                  = $data['total_cost'];
                    }
                }else{
                    $mergeRows[$name][$data['company']][$data['category']]['total_cost_column_id']    = $totalCostId;
                    $mergeRows[$name][$data['company']][$data['category']]['array_ids'][]             = $data[0];
    ...
    return ['result' => $result, 'mergeRows' => $mergeRows, 'dataToSum' => $dataToSum];
    }
    ...
    public function postCustom(Request $request)
    {
        ...
        $response = new StreamedResponse(function () use ($customfields, $request) {
            ...
            'Content-Disposition' => 'attachment; filename="custom-assets-report-'.date('Y-m-d-his').'.xlsx"',
        ]);
