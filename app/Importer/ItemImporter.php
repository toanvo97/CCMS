...
class ItemImporter extends Importer
{
    ...
    protected function handle($row)
    {
        ...
        if ($this->findCsvMatch($row, 'purchase_date') != '') {
            $purchase_date = str_replace('/', '-', $this->findCsvMatch($row, 'purchase_date'));
            $this->item['purchase_date'] = date('Y-m-d 00:00:01', strtotime($purchase_date));
        }

        $this->item['last_audit_date'] = null;
        if ($this->findCsvMatch($row, 'last_audit_date') != '') {
            $last_audit_date = str_replace('/', '-', $this->findCsvMatch($row, 'last_audit_date'));
            $this->item['last_audit_date'] = date('Y-m-d', strtotime($last_audit_date));
        }

        $this->item['next_audit_date'] = null;
        if ($this->findCsvMatch($row, 'next_audit_date') != '') {
            $next_audit_date = str_replace('/', '-', $this->findCsvMatch($row, 'next_audit_date'));
            $this->item['next_audit_date'] = date('Y-m-d', strtotime($next_audit_date));
        }
    ...
    {
        $condition = array();
        $category = Category::where('name',$this->findCsvMatch($row,'category'))->first();
