...
<div class="row">
  <div class="col-md-8 col-md-offset-2">
    ...
    <form class="form-horizontal" id="customForm" action="{{route('custom.export')}}" method="get">
      {{csrf_field()}}
          <!-- Horizontal Form -->
          <div class="box box-default">
            ...
            <div class="box-body">
              <div class="col-md-4">
                ...
                <div class="checkbox col-md-12">
                  <label>
                    {{ Form::checkbox('total_cost', '1', '1', ['class' => 'minimal']) }}
                    {{ trans('admin/hardware/form.total_cost') }}
                  </label>
                </div>
                {{-- <div class="checkbox col-md-12">
                  <label>
                ...
                {{-- @if ($customfields->count() > 0)
                  <div class="checkbox col-md-12">
                    <h2>{{ trans('admin/custom_fields/general.custom_fields') }}</h2>:</h4>
                  </div>
              ...
              @include ('partials.forms.edit.model-select', ['translated_name' => trans('general.asset_models'), 'fieldname' => 'by_model_id', 'hide_new' => 'true'])
              ...
              <!-- Status -->
                ...
                <div class="form-group">
                  <label for="by_asset_name" class="col-md-3 control-label">{{ trans('admin/hardware/form.name') }}</label>
                  <div class="col-md-5 col-sm-8">
                    <input class="form-control" type="text" name="by_asset_name" value="" aria-label="by_asset_name">
                  </div>
                </div>
                ...
                <div class="form-group">
                  <label for="by_notes" class="col-md-3 control-label">{{ trans('general.notes') }}</label>
                  <div class="col-md-5 col-sm-8">
                    <input class="form-control" type="text" name="by_notes" value="" aria-label="by_notes">
                  </div>
                </div>
              ...
              <!-- Purchase Date -->
                ...
                <div class="col-md-9 col-md-offset-3">
                  <label>
                    {{ Form::checkbox('use_bom', '1', '1', ['class' => 'use_bom', 'id' => 'use_bom', 'data-id' => 'use_bom']) }}
                ...
                <div class="col-md-9 col-md-offset-3">
                  <label>
                    {{ Form::checkbox('use_bom_model', '1', '0', ['class' => 'use_bom', 'id' => 'use_bom_model', 'data-id' => 'use_bom_model']) }}
                    {{ trans('general.bom_model') }}
                  </label>
    
                </div>
                <div class="col-md-9 col-md-offset-3">
                  <label>
                    {{ Form::checkbox('use_bom_location', '1', '0', ['class' => 'use_bom', 'id' => 'use_bom_location', 'data-id' => 'use_bom_location']) }}
                    {{ trans('general.bom_location') }}
                  </label>
    
                </div>
    
              </div>
              <input type="text" name="export_type" value="all" hidden>
            </div> <!-- /.box-body-->
            <div class="box-footer text-right">
              <button id="btnCustom" class="btn btn-success"><i class="fas fa-download icon-white" aria-hidden="true"></i> {{ trans('general.generate') }}</button>
            </div>
          </div> <!--/.box.box-default-->
...
@section('moar_scripts')
  <script>
    ...
    // Check-all / Uncheck all
      ...
      $('#btnCustom').on('click', function(e){
        console.log($('input[name=asset_name]'));
        if($('input.minimal').filter(':checked').length < 2){
          e.preventDefault();
          alert('Vui lòng chọn tối thiểu 2 dữ liệu cần xuất báo cáo');
        }
        else if($('input[name=asset_name]').prop('checked') == false){
          e.preventDefault();
          alert('Vui lòng chọn tên tài sản để xuất báo cáo');
        }
        else if($('input[name=category]').prop('checked') == false){
          e.preventDefault();
          alert('Vui lòng chọn thể loại để xuất báo cáo');
        }
        else if($('input[name=order]').prop('checked') == false){
          e.preventDefault();
          alert('Vui lòng chọn số lượng để xuất báo cáo');
        }
        else if($('input[name=total_cost]').prop('checked') == false){
          e.preventDefault();
          alert('Vui lòng chọn thành tiền để xuất báo cáo');
        }
        else{
          $('#customForm').submit();
        }
      ...
      $('#use_bom').click(function(){
        let check = $(this)
        if($(this).prop('checked') == true){
          $('.use_bom').each(function(){
            if($(this).prop('checked') == true && check.attr('data-id') != $(this).attr('data-id')){
      ...
      $('#use_bom_location').click(function(){
        let check = $(this)
        if($(this).prop('checked') == true){
          $('.use_bom').each(function(){
            if($(this).prop('checked') == true && check.attr('data-id') != $(this).attr('data-id')){
