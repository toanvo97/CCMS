...
@section('inputFields')
  ...
  @include ('partials.forms.edit.total_cost', ['currency_type' => $currency_type])
...
<script nonce="{{ csrf_token() }}">
    ...
    $(document).ready(function(){
        let amount = 0;
        let cost = 0;
        const totalExist = $('#total_cost').val();
        $('#purchase_cost').keyup(function(){
            cost = $(this).val().replace(/[$,]+/g,"");
            amount = $('#order_number').val();
            $(this).val(formatCurrency(cost)).trigger('change');
            calculateSum(amount, cost);
        });

        function formatCurrency($cost){
            let formatedCost = new Intl.NumberFormat().format($cost);
            return formatedCost;
        }

        $('#order_number').keyup(function(){
            amount = $(this).val();
            cost = $('#purchase_cost').val().replace(/[$,]+/g,"");
            calculateSum(amount, cost);
        });
        function calculateSum(amount, cost) {
            let total = 0;
                //add only if the value is number
                if(!isNaN(amount) && !isNaN(cost) && cost.length!=0 && amount.length!=0) {
                    total = amount*cost;
                }
            if(total != 0){
                $("#total_cost").val(formatCurrency(total)).trigger('change');
            }
            else{
